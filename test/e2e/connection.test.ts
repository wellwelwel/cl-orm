...
const servers = [
  { name: 'D1', script: 'dev:d1', port: 8787 },
  { name: 'Durable Objects', script: 'dev:do', port: 8788 },
];

describe('Starting drivers', async () => {
  for (const { name, script, port } of servers) {
    await kill.port(port);

    const BASE_URL = `http://localhost:${port}`;
    const server = await startScript(script, {
      startAfter: 'Ready on',
      timeout: 5000,
      verbose: true,
    });
